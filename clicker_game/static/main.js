...
/* ===== 本地儲存 ===== */
const collected = JSON.parse(localStorage.getItem('collection') || '{}');
...
imageEl.addEventListener('click', () => {
  ...
  if (chance < 2)        showParticles(cx, cy, true);   // 傳說稀有：爆炸粒子
  else if (chance < 3)   showParticles(cx, cy, false);  // 機稀有：普通粒子
...
// ======= 抽卡次數限制邏輯 ======= //
const MAX_COUNT = 100;
const RECOVERY_INTERVAL = 60 * 1000; // 1 分鐘
const RECOVERY_AMOUNT = 3;

function loadCounterData() {
  const data = JSON.parse(localStorage.getItem('drawCounter')) || {
    count: MAX_COUNT,
    lastUpdate: Date.now()
  };
  return data;
...
function saveCounterData(data) {
  localStorage.setItem('drawCounter', JSON.stringify(data));
}

function recoverCounter(data) {
  const now = Date.now();
  const elapsed = now - data.lastUpdate;
  const recoverTimes = Math.floor(elapsed / RECOVERY_INTERVAL);

  if (recoverTimes > 0) {
    data.count = Math.min(MAX_COUNT, data.count + recoverTimes * RECOVERY_AMOUNT);
    data.lastUpdate += recoverTimes * RECOVERY_INTERVAL;
...
function updateDrawDisplay(data) {
  document.getElementById('draw-counter').textContent = `抽卡次數：${data.count}`;
  updateRecoveryTimer(data);
}

function updateRecoveryTimer(data) {
  const now = Date.now();
  const nextRecoverIn = Math.max(0, RECOVERY_INTERVAL - (now - data.lastUpdate));
  const seconds = Math.ceil(nextRecoverIn / 1000);
  const timerEl = document.getElementById('recovery-timer');

  if (data.count >= MAX_COUNT) {
    timerEl.textContent = '已達上限，無法再恢復';
  } else {
    timerEl.textContent = `下一次恢復還有 ${seconds} 秒`;
  }
}

// 初始化次數
let counterData = recoverCounter(loadCounterData());
updateDrawDisplay(counterData);

// 每秒更新倒數與檢查恢復
setInterval(() => {
  counterData = recoverCounter(counterData);
  updateDrawDisplay(counterData);
}, 1000);
